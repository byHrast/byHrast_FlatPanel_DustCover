...
void setup() {
  ...
  server.on("/HEATERstatus.htm", HTTP_GET, [](AsyncWebServerRequest * request) {
    ...
    for (int i = 0; i < paramsNr; i++) {
      ...
      if (p->name() == "heater") {
        ...
        if (var1 == "1") ledON(255);/// zamjeniti ovo s fjom gdje gasi grijanje
      }
    }
  });
  server.on("/COVERstatus.htm", HTTP_GET, [](AsyncWebServerRequest * request) {
    ...
    for (int i = 0; i < paramsNr; i++) {
      ...
      if (p->name() == "cover") {
        var1 = p->value();
        if (var1 == "1") {} //openCover(); /// zamjeniti ovo s fjom gdje otvara cover
        if (var1 == "0") {} //closeCover();/// zamjeniti ovo s fjom gdje zatvara cover
      }
    }
  });
  // Start server
...
void closeCover() {
  ...
  if (coverState != "CLOSED") {    
    for (pos = 0; pos <= 180; pos++) {  // goes from 0 degrees to 180 degrees9
